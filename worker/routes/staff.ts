...
// List all staff members (Available to authenticated staff to show assigned names, full management to admin)
staffApp.get('/', async (c) => {
  try {
    ...
    // For non-admin, hide sensitive details like other users' passwords
    return c.json({
      success: true,
      staff: users.results || [],
    ...
    return c.json({ success: false, error: 'Failed to fetch staff list.' }, 500);
  }
});

// Single staff details with recent activity
staffApp.get('/:id', async (c) => {
  try {
    ...
    const recentAuditLogs = await db
      .prepare(
        `SELECT * FROM audit_logs WHERE staff_id = ? ORDER BY created_at DESC LIMIT 20`
      )
      .bind(staffId)
      .all<any>();
    ...
    const recentTransactions = await db
      .prepare(
        `SELECT t.*, c.full_name AS customer_name FROM transactions t JOIN customers c ON t.customer_id = c.id WHERE t.staff_id = ? ORDER BY t.date DESC LIMIT 10`
      )
      .bind(staffId)
      .all<any>();
...
staffApp.post('/', adminOnlyMiddleware, async (c) => {
  try {
    const db = c.env.DB;
    const currentUser = c.get('user');
...
staffApp.put('/:id', adminOnlyMiddleware, async (c) => {
  try {
    const db = c.env.DB;
    const currentUser = c.get('user');
    const staffId = c.req.param('id');
