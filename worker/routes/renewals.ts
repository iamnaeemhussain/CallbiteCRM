...
renewalsApp.get('/', async (c) => {
  try {
    ...
    return c.json({ success: false, error: 'Failed to fetch renewals list.' }, 500);
  }
});
...
renewalsApp.post('/', async (c) => {
  try {
    ...
    const timelineDesc = `${currentUser.name} renewed ${body.package_name || existingEsim.package_name}. Expiry extended from ${existingEsim.expiry_date} to ${body.new_expiry_date}. Payment of $${sellPrice.toFixed(2)} via ${body.payment_method || 'Cash'}.`;
