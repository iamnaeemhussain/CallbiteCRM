...
dashboardApp.get('/stats', async (c) => {
  try {
    ...
    // 5. Transaction & Financial Statistics (in PKR)
    ...
    const salesTrend = recent7Days.map((day) => {
      const item = salesTrendMap.get(day);
      return {
        date: day,
        sales: item ? item.sales || 0 : 0,
        profit: item ? item.profit || 0 : 0,
    ...
    return c.json({
      success: true,
      customers: { total: 0, active: 0, inactive: 0, vip: 0, blocked: 0, new_this_month: 0 },
      esims: { total: 0, active: 0, pending: 0, expired: 0, suspended: 0, cancelled: 0 },
      renewals: { expired: 0, expiring_today: 0, expiring_3_days: 0, expiring_7_days: 0, active_healthy: 0 },
      support: { total: 0, open: 0, in_progress: 0, waiting_customer: 0, resolved_today: 0, urgent_open: 0 },
      transactions: { today_sales: 0, today_profit: 0, month_sales: 0, month_profit: 0, month_renewals_count: 0, total_revenue: 0, total_profit: 0 },
      attention: { overdue_tasks: [], today_tasks: [], expiring_esims: [], urgent_tickets: [], recent_activity: [] },
      sources: [],
      sales_trend: [],
    });
  }
