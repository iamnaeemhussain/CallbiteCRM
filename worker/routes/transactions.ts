...
transactionsApp.get('/', async (c) => {
  try {
    ...
    // Overall summary metrics across matching transactions (excluding pagination)
    let summaryQuery = `
      SELECT 
...
transactionsApp.post('/', async (c) => {
  try {
    ...
    await logTimeline(db, {
      ...
      title: `${body.transaction_type || 'Transaction'}: $${sellPrice.toFixed(2)}`,
      description: `${currentUser.name} recorded ${body.payment_status || 'Paid'} ${body.transaction_type} payment of $${sellPrice.toFixed(2)} via ${body.payment_method || 'Cash'}.`,
    ...
    await logAudit(db, {
      ...
      change_summary: `Recorded ${body.transaction_type} ($${sellPrice.toFixed(2)}) for ${customer.full_name}`,
      ip_address: clientIp,
    });
...
transactionsApp.delete('/:id', async (c) => {
  try {
    ...
    await logAudit(db, {
      ...
      record_id: txnId,
      previous_value: existing,
      change_summary: `${currentUser.name} deleted transaction ${txnId} ($${existing.selling_price})`,
      ip_address: clientIp,
    });
