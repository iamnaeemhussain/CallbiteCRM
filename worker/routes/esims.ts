...
// List eSIMs with search & filters
esimsApp.get('/', async (c) => {
  try {
    ...
    const sortCol = validSorts[sort_by] || 'e.created_at';
    ...
    return c.json({ success: false, error: 'Failed to fetch eSIMs.' }, 500);
  }
});
...
esimsApp.post('/', async (c) => {
  try {
    ...
    const esimId = await generateId(db, 'esims', 'ESIM', 2019);
    ...
    if (body.record_transaction && body.selling_price !== undefined && body.selling_price >= 0) {
      const txnId = await generateId(db, 'transactions', 'TXN', 3020);
