...
customersApp.get('/:id', async (c) => {
  try {
    ...
    const tagsRes = await db
      .prepare(`SELECT tag_name FROM customer_tags WHERE customer_id = ?`)
      .bind(customerId)
      .all<{ tag_name: string }>();
    customer.tags = (tagsRes.results || []).map((t) => t.tag_name);

    // 2. All eSIMs for this customer
    const esims = await db
      .prepare(
        `SELECT e.*, u.name AS created_by_staff_name
         FROM esims e
         LEFT JOIN users u ON e.created_by_staff_id = u.id
         WHERE e.customer_id = ? AND e.is_deleted = 0
         ORDER BY e.created_at DESC`
      )
      .bind(customerId)
      .all<any>();

    // 3. Purchase History / Transactions
    const transactions = await db
      .prepare(
        `SELECT t.*, u.name AS staff_name, e.iccid AS esim_iccid
         FROM transactions t
    ...
    const supportTickets = await db
      .prepare(
        `SELECT s.*, u.name AS assigned_staff_name, cu.name AS created_by_staff_name, e.package_name AS esim_package, e.iccid AS esim_iccid
    ...
    // 5. Contact History / Interactions
    ...
    const tasks = await db
      .prepare(
        `SELECT t.*, u.name AS assigned_staff_name, cu.name AS created_by_staff_name, e.package_name AS esim_package
         FROM tasks t
    ...
    // 9. Customers referred by this customer (if any)
    const referredCustomers = await db
      .prepare(
        `SELECT c.id, c.full_name, c.whatsapp_number, c.status, c.created_at,
          (SELECT COUNT(*) FROM esims e WHERE e.customer_id = c.id AND e.is_deleted = 0) AS esim_count
         FROM customers c
         WHERE c.referred_by_customer_id = ? AND c.is_deleted = 0
         ORDER BY c.created_at DESC`
      )
      .bind(customerId)
      .all<any>();

    // 10. Customer Lifetime Metrics
...
customersApp.post('/', async (c) => {
  try {
    ...
    // Auto-log creation timeline & audit
    ...
    if (body.initial_esim && body.initial_esim.iccid) {
      ...
      await db
        .prepare(
          `INSERT INTO esims (
            id, customer_id, iccid, country_region, provider, package_name,
            data_allowance, duration, start_date, expiry_date, status,
            qr_code_data, apn_info, notes, created_by_staff_id, is_deleted,
      ...
      if (esim.selling_price !== undefined && esim.selling_price > 0) {
        ...
        await logTimeline(db, {
          ...
          title: `Purchase Recorded ($${sell.toFixed(2)})`,
          description: `Initial eSIM purchase recorded via ${esim.payment_method || 'Cash'}.`,
